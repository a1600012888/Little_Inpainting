...
class Cityscapes_dataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        ori_img = cv.resize(ori_img, (128, 256))
        ori_img = center_crop(ori_img, (128, 128))
        mask = np.zeros((ori_img.shape[0], ori_img.shape[1], 1))

        sy = np.random.randint(0, 128 - 32)     # np.random does not support multi-thread
        sx = np.random.randint(0, 128 - 32)    # set worker_init_fn for dataloader
        ey = sy + 32
        ex = sx + 32
